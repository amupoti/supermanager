    ...
    <tbody align="center">
        ...
        <#list teamData.rows as row>
            <#if !row.slotRow>
                ...
                <tr>
                    <td align="left">${p.name}<#if p.status.injured> <span class="label label-danger">LES</span></#if><#if p.status.postponed> <span class="label label-warning">APL</span></#if><#if p.status.doubtful> <span class="label label-info">DUD</span></#if><#if p.status.blocked> <span class="label label-default">BLQ</span></#if></td>
